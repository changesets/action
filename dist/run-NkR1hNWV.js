...
`)),c}async function ue({script:e,github:t,cwd:n=process.cwd(),prTitle:i=`Version Packages`,commitMessage:a=`Version Packages`,hasPublishScript:o=!1,prBodyMaxCharacters:s=6e4,branch:f=h.ref.replace(`refs/heads/`,``),prDraft:g}){let{octokit:v}=t,y=`changeset-release/${f}`,{preState:b}=await _(n);await t.prepareBranch(y);let C=await c(n),w={...process.env,GITHUB_TOKEN:t.getToken()};e?await r(e,void 0,{cwd:n,env:w}):await p([`version`],{cwd:n,env:w});let T=await m(n,C),E=Promise.all(T.map(async e=>{let t=await x.readFile(S.join(e.dir,`CHANGELOG.md`),`utf8`),n=u(t,e.packageJson.version);return{highestLevel:n.highestLevel,private:!!e.packageJson.private,content:n.content,header:`## ${e.packageJson.name}@${e.packageJson.version}`}})),D=`${i}${b?` (${b.tag})`:``}`,O=`${a}${b?` (${b.tag})`:``}`,k=await v.rest.pulls.list({...h.repo,state:`open`,head:`${h.repo.owner}:${y}`,base:f});d(`Existing pull requests: ${JSON.stringify(k.data,null,2)}`),await t.pushChanges({branch:y,message:O});let A=await le({hasPublishScript:o,preState:b,branch:f,changedPackagesInfo:(await E).filter(e=>e).sort(l),prBodyMaxCharacters:s});if(k.data.length===0){d(`creating pull request`);let{data:e}=await v.rest.pulls.create({base:f,head:y,title:D,body:A,draft:g!==void 0,...h.repo});return{pullRequestNumber:e.number}}{let[e]=k.data;d(`updating found pull request #${e.number}`);let t=`
